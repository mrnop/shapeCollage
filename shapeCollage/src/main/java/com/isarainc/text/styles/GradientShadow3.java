...
public class GradientShadow3 extends TextStyle {
	...
	protected void drawText(Canvas canvas, Path path, String text) {
		canvas.drawTextOnPath(text, path, 0, 1, mTextPaintShadow);
	...
	protected void drawText(Canvas canvas, int x, int y, String text) {
		...
		canvas.drawText(text, x, y, mTextPaintShadow);
	...
	protected void prepare(Canvas canvas, String text) {
		...
		mTextPaintShadow.setTypeface(getTypeface());
		mTextPaintShadow.setAntiAlias(true);
		mTextPaintShadow.setTextSize(getSize());
		mTextPaintShadow.setTextAlign(Align.CENTER);
		mTextPaintShadow.setColor(textColor3);
		mTextPaintShadow.setStyle(Paint.Style.FILL);
		float radius=this.getSize()/5;
		float d=this.getSize()/10;
		mTextPaintShadow.setShadowLayer(radius, d, d, textColor3);
